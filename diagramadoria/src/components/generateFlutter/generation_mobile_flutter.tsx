...
const getJPARelationType = (
  ...
  if (relationType === 'herencia') {
    return { fromSide: null, toSide: 'Inheritance' };
  }
...
const processRelations = (relations: UMLRelation[], classes: UMLClassNode[]): ProcessedRelation[] => {
  ...
  relations.forEach(rel => {
    ...
    const { fromSide, toSide } = getJPARelationType(rel.type, rel.originCard, rel.destCard);

    if (rel.type === 'herencia') {
      processed.push({
        ownerId: rel.fromDisplayId,
        relatedId: rel.toDisplayId,
        ownerClassName: fromName,
        relatedClassName: toName,
        fieldName: 'parent',
        type: 'Inheritance',
        isOwner: true,
        umlType: rel.type
    ...
    if (fromSide === 'OneToMany' && toSide === 'ManyToOne') {
      processed.push({
        ownerId: rel.toDisplayId,
        relatedId: rel.fromDisplayId,
        ownerClassName: toName,
        relatedClassName: fromName,
        fieldName: fromName.charAt(0).toLowerCase() + fromName.slice(1),
        type: 'ManyToOne',
        isOwner: true,
        umlType: rel.type
      });
      processed.push({
        ...
        ownerClassName: fromName,
        relatedClassName: toName,
        fieldName: toName.charAt(0).toLowerCase() + toName.slice(1) + 's',
        type: 'OneToMany',
        isOwner: false,
        inverse: fromName.charAt(0).toLowerCase() + fromName.slice(1),
        umlType: rel.type
      });
      ...
      processed.push({
        ...
        ownerClassName: fromName,
        relatedClassName: toName,
        fieldName: toName.charAt(0).toLowerCase() + toName.slice(1),
        type: 'ManyToOne',
        isOwner: true,
        umlType: rel.type
      });
      processed.push({
        ...
        ownerClassName: toName,
        relatedClassName: fromName,
        fieldName: fromName.charAt(0).toLowerCase() + fromName.slice(1) + 's',
        type: 'OneToMany',
        isOwner: false,
        inverse: toName.charAt(0).toLowerCase() + toName.slice(1),
        umlType: rel.type
      });
      ...
      processed.push({
        ...
        type: 'ManyToMany',
        isOwner: false,
        inverse: toName.charAt(0).toLowerCase() + toName.slice(1) + 's',
        umlType: rel.type
      });
    }
  });
...
const genModelDart = (c: UMLClassNode, relations: ProcessedRelation[]) => {
  ...
  // Campos simples
  const fields = c.attributes.map(a => {
    const field = toFieldName(a.name);
    if (field === 'id') return '  int? id;';
    return `  ${mapUMLTypeToDart(a.type)}? ${field};`;
  });

  // Campos relaciones como IDs
  const rels = relations.filter(r => r.ownerId === c.displayId);
  rels.forEach(r => {
    if (r.type === 'ManyToOne' || (r.type === 'OneToOne' && r.isOwner) || (r.type === 'OneToOne' && !r.isOwner)) {
      fields.push(`  int? ${r.fieldName}Id;`);
    } else if (r.type === 'OneToMany' || r.type === 'ManyToMany') {
      fields.push(`  List<int> ${r.fieldName}Ids = const [];`);
    }
  });

  const ctorParams = fields.map(f => {
    const fname = f.trim().replace(/;$/, '').split(' ').pop();
    return `this.${fname},`;
  }).join(' ');
...
// lib/screens/{entity}/{entity}_list.dart
const genListScreen = (c: UMLClassNode) => {
  ...
  // título para card: primer atributo String o 'id'
                   ...
                   return ListTile(
                     ...
                     trailing: Row(
                       ...
                       children: [
                         ...
                         IconButton(icon: const Icon(Icons.delete), onPressed: () => _delete(it.id!)),
                       ],
                     ),
                   );
                 },
...
// lib/screens/{entity}/{entity}_form.dart
const genFormScreen = (c: UMLClassNode, relations: ProcessedRelation[]) => {
  ...
  simpleAttrs.forEach(a => {
    ...
    if (dartType === 'int') {
      ...
      // Campo bool no generado con switch aquí; si lo necesitas, añade UI de bool
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          FilledButton(onPressed: _save, child: Text(isEdit ? 'Guardar' : 'Crear')),
        ],
      ),
    );
  }
 ...
 * Componente React
 * ============================== */
const GenerationMobileFlutter: React.FC = () => {
    ...
    try {
      ...
      if (projectData && projectData.diagrama_json) {
        ...
        if (projectData.name) {
          ...
          setCfg(prev => ({
            ...prev,
            appName: `${projectData.name} Mobile`,
            appId: `com.example.${clean}`,
          }));
        }
      } else {
...
export default GenerationMobileFlutter;